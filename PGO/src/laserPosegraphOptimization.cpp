...
void initNoises(void) {
    ...
    robustLoopNoise = gtsam::noiseModel::Robust::Create(
            gtsam::noiseModel::mEstimator::Cauchy::Create(
                    1), // optional: replacing Cauchy by DCS or GemanMcClure is okay but Cauchy is empirically good.
...
Pose6D diffTransformation(const Pose6D &_p1, const Pose6D &_p2) {
    ...
    return Pose6D{double(abs(dx)), double(abs(dy)), double(abs(dz)), double(abs(droll)), double(abs(dpitch)),
                  double(abs(dyaw))};
} // SE3Diff
...
void pubPath(void) {
    ...
    for (int node_idx = 0; node_idx <
                           recentIdxUpdated; node_idx++) // -1 is just delayed visualization (because sometimes mutexed while adding(push_back) a new one)
    {
        ...
        odomAftPGOthis.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(pose_est.roll, pose_est.pitch,
                                                                                       pose_est.yaw);
    ...
    tf::Quaternion q;
    transform.setOrigin(tf::Vector3(odomAftPGO.pose.pose.position.x, odomAftPGO.pose.pose.position.y,
...
gtsam::Pose3 doICPVirtualRelative(int _loop_kf_idx, int _curr_kf_idx) {
    ...
    // Pose6dToAffine3f(pose): return pcl::getTransformation(pose.x, pose.y, pose.z, pose.roll, pose.pitch, pose.yaw);
    ...
    // 得到的是poseTo相对于poseFrom的姿态变化，即以poseFrom为基准坐标，poseTo在poseFrom基准坐标下的坐标。
...
void process_pg() {
    while (1) {
        while (!odometryBuf.empty() && !fullResBuf.empty()) {
            ...
            //
            pcl::PointCloud<PointType>::Ptr thisKeyFrameDS(new pcl::PointCloud<PointType>()); // 降采样之后的关键帧
            ...
            const int curr_node_idx = keyframePoses.size() -
                                      1; // becuase cpp starts with 0 (actually this index could be any number, but for simple implementation, we follow sequential indexing)
            if (!gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    //// 为保证实时性，注释ISAM2优化过程 ，将这一过程放到单独的线程中（isam_update）
                    // runISAM2opt();
                }
                mtxPosegraph.unlock();
                ...
                mtxPosegraph.lock();
                {
                    // odom factor
                    gtSAMgraph.add(
                            gtsam::BetweenFactor<gtsam::Pose3>(prev_node_idx, curr_node_idx, poseFrom.between(poseTo),
                                                               odomNoise));
...
bool detectLoopClosureDistance(int *loopKeyCur, int *loopKeyPre) {
    ...
    // 在原始点云与目标点云重叠区域较小时，性能：radiusSearch > neatestKSearch
...
void performRSLoopClosure(void) {
    ...
    if (detectLoopClosureDistance(&loopKeyCur, &loopKeyPre)) {
        ...
        loopIndexContainer[loopKeyCur] = loopKeyPre; // 更新loop_idx
        // addding actual 6D constraints in the other thread, icp_calculation.
        mBuf.unlock();
    } else
        return;
    ...
    while (1) {
        while (!scLoopICPBuf.empty()) {
            ...
            if (!relative_pose.equals(gtsam::Pose3::identity())) {
                ...
                mtxPosegraph.lock();
                gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(curr_node_idx, prev_node_idx, relative_pose,
                                                                  robustLoopNoise));
                // runISAM2opt();
                mtxPosegraph.unlock();
            }
...
int main(int argc, char **argv) {
    ...
    nh.param<double>("sc_max_radius", scMaximumRadius,
                     80.0); // 80 is recommended for outdoor, and lower (ex, 20, 40) values are recommended for indoor
    ...
    /*** subscriber ***/
    ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_cloud_registered_local",
                                                                                  100, laserCloudFullResHandler);
    ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/aft_mapped_to_init", 100,
                                                                        laserOdometryHandler);
    ...
    std::thread isam_update{
            process_isam}; // if you want to call less isam2 run (for saving redundant computations and no real-time visulization is required), uncommment this and comment all the above runisam2opt when node is added.
